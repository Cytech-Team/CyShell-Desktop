...
test("focus discovery prefers native output, then recent interaction and active toplevel before first-screen fallback", () => {
    const source = read("Services/CompositorService.qml");
    const context = vm.createContext({
        isAqueous: false, isHyprland: false, isNiri: false, isSway: false, isScroll: false, isMiracle: false, isMango: false, isLabwc: true,
    ...
    context.ToplevelManager.toplevels.values = [{ activated: true, screens: [{ name: "second" }] }];
    assert.equal(context.getFocusedScreenName(), "second", "generic/Labwc active toplevel output");
    assert.equal(context.getFocusedScreen().name, "second");

    context.lastInteractionScreenName = "first";
    context.lastInteractionScreenTimestamp = Date.now();
    assert.equal(context.getFocusedScreenName(), "first", "recent explicit shell interaction wins generic fallback");
    context.lastInteractionScreenTimestamp = 0;
    ...
    context.ToplevelManager.toplevels.values = [];
    context.lastInteractionScreenName = "";
...
test("system updater IPC does not require the optional updater bar widget", () => {
    ...
    const handler = source.slice(handlerStart, target);
    assert.ok(handler.includes("const bar = root.getPreferredBar();"));
    assert.ok(!handler.includes('getPreferredBar("systemUpdateButtonRef")'));
});
