...
Item {
    ...
    readonly property var _agentApprovalServiceRef: AgentApprovalService
    ...
    LazyLoader {
        ...
        AppPickerModal {
            ...
            onApplicationSelected: (app, filePath) => {
                ...
                SessionService.launchDetachedApp(["sh", "-c", cmd], Quickshell.env("HOME"), {});
            }
        }
    }
    ...
    LazyLoader {
        id: agentApprovalModalLoader
        active: false

        Component.onCompleted: PopoutService.agentApprovalModalLoader = agentApprovalModalLoader

        AgentApprovalModal {
            id: agentApprovalModal

            Component.onCompleted: PopoutService.agentApprovalModal = agentApprovalModal
            onClosingModal: Qt.callLater(PopoutService.unloadAgentApproval)
        }
    }

    LazyLoader {
        id: agentAssistantModalLoader
        active: false

        Component.onCompleted: PopoutService.agentAssistantModalLoader = agentAssistantModalLoader

        AgentAssistantModal {
            id: agentAssistantModal

            Component.onCompleted: PopoutService.agentAssistantModal = agentAssistantModal
            onClosingModal: Qt.callLater(PopoutService.unloadAgentAssistant)
        }
    }
