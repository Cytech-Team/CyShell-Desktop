...
Item {
    ...
    Rectangle {
        ...
        color: Theme.withAlpha(Theme.surfaceText, Theme.stateLayerHover)
        opacity: root.options.taskbarVisuals && root.isHovered ? 1 : 0
        visible: root.options.taskbarVisuals ?? false
        z: -2
    ...
    Rectangle {
        anchors.fill: parent
        radius: Theme.cornerRadiusS
        color: root.options.taskbarVisuals ? Theme.withAlpha(Theme.surfaceText, Theme.stateLayerPressed) : Theme.withAlpha(root.activeColor, Theme.stateLayerPressed)
    ...
    DockHoverBounce {
        ...
        suppressed: mouseArea.pressed || root.dragging || (root.options.taskbarVisuals ?? false)
            ...
            Repeater {
                ...
                Rectangle {
                    width: {
                        ...
                        if (root.options.indicatorStyle === "taskbar")
                            return appData && appData.type === "grouped" && appData.windowCount > 1 ? Math.max(3, actualIconSize * 0.12) : Math.max(12, actualIconSize * 0.55);
                    ...
                    height: {
                        if (root.options.indicatorStyle === "circle")
                            return Math.max(4, actualIconSize * 0.1);
                        if (root.options.indicatorStyle === "taskbar")
                            return Math.max(2, actualIconSize * 0.07);
