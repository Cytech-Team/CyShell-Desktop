...
Item {
    ...
    readonly property real visualTargetSize: barHosted ? iconSize * (options.iconSizePercentage ?? 100) / 100 : iconSize
    // Taskbar buttons need a real pointer target, not an icon-sized target. Keep the
    // icon compact while guaranteeing a Windows-like 36px slot and the full bar
    // thickness on the cross axis. This removes dead strips around AppsDock icons.
    readonly property real targetSize: barHosted && (options.taskbarVisuals ?? false) ? Math.max(36, visualTargetSize) : visualTargetSize
    ...
    readonly property real crossSize: {
        ...
        if (!root.barHosted)
            return needed;
        if (root.options.taskbarVisuals ?? false)
            return Math.max(root.surfaceContext?.thickness ?? needed, root.surfaceContext?.widgetThickness ?? needed);
        // A normal bar pill cannot grow past its widget thickness, so the lane yields there.
