...
Item {
    ...
    readonly property var greeterActionCommand: greeterAction === "activate" ? ["cyshell-greeter", "enable", "--terminal"] : []
    ...
    function promptGreeterActionConfirm() {
        ...
        greeterActionConfirm.showWithOptions({
            ...
            "message": I18n.tr("Activate the CyShell greeter compatibility backend? A terminal will open for sudo authentication. Run Sync after activation to apply your settings."),
    ...
    Process {
        ...
        command: ["sh", "-c", "command -v cyshell-greeter >/dev/null 2>&1"]
        running: false
    ...
    Process {
        ...
        command: ["cyshell-greeter", "status"]
        running: false
        ...
        onExited: exitCode => {
            ...
            var failure = I18n.tr("Failed to run 'cyshell-greeter status'. Ensure the cyshell-greeter package is installed.", "greeter status error") + " (exit " + exitCode + ")";
            if (out !== "")
                failure = failure + "\n\n" + out;
            if (err !== "")
                failure = failure + "\n\nstderr:\n" + err;
    ...
    Process {
        ...
        command: ["cyshell-greeter", "sync", "--yes"]
        running: false
    ...
    Process {
        ...
        command: ["cyshell-greeter", "sync", "--terminal", "--yes"]
        running: false
        ...
        onExited: exitCode => {
            ...
            var fallback = I18n.tr("Terminal fallback failed. Install one of the supported terminal emulators or run 'cyshell-greeter sync' manually.") + " (exit " + exitCode + ")";
    ...
    SettingsPage {
        ...
        SettingsCard {
            ...
            SettingsRow {
                body: StyledText {
                    text: I18n.tr("Sync applies your theme and settings to the login screen. Shared users should run cyshell-greeter sync --profile instead of a primary user sync.")
                    ...
                    StyledText {
                        ...
                        text: {
                            ...
                            if (root.embeddedGreeterOnly)
                                return I18n.tr("The legacy greeter backend is active (archinstall setup). It keeps working as is, but syncing CyShell theme and settings needs the standalone compatibility greeter. Use the bundled CyShell greeter launcher, then run Sync to migrate the login screen.", "embedded greeter status");
                            if (!root.greeterBinaryExists && root.greeterEnabled)
                                return I18n.tr("CyShell greeter launcher is missing. Reinstall or update CyShell to restore it.", "greeter status placeholder");
        ...
        SettingsCard {
            ...
            SettingsRow {
                body: StyledText {
                    text: I18n.tr("Requires greetd, cyshell-greeter, and your user in the greeter group (plus fprintd/pam_fprintd for fingerprint, pam_u2f for security keys).")
                    font.pixelSize: Theme.fontSizeSmall
                    color: Theme.surfaceVariantText
                    width: parent.width
                    wrapMode: Text.Wrap
