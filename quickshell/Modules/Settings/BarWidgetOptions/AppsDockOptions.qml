...
Column {
    ...
    SettingsCard {
        ...
        SettingsButtonGroupRow {
            resetStore: root.dockHosted ? root.apps : root.page.store
            resetKeys: [root.dockHosted ? "indicatorStyle" : "appsDockIndicatorStyle"]
            text: I18n.tr("Indicator style")
            enabled: !root.page.value("appsDockHideIndicators")
            model: root.dockHosted ? [I18n.tr("Circle", "dock indicator style option"), I18n.tr("Line", "dock indicator style option")] : [I18n.tr("Circle", "dock indicator style option"), I18n.tr("Line", "dock indicator style option"), I18n.tr("Taskbar", "dock indicator style option")]
            buttonPadding: Theme.spacingS
            currentIndex: {
                const value = root.dockHosted ? root.apps.get("indicatorStyle") : root.page.value("appsDockIndicatorStyle");
                return value === "circle" ? 0 : value === "taskbar" && !root.dockHosted ? 2 : 1;
            }
            onSelectionChanged: (index, selected) => {
                if (!selected)
                    return;
                const value = index === 0 ? "circle" : index === 2 && !root.dockHosted ? "taskbar" : "line";
                if (root.dockHosted)
                    root.apps.set("indicatorStyle", value);
                else
                    root.page.set("appsDockIndicatorStyle", value);
            }
        }
