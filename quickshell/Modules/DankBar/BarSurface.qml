...
Item {
    ...
    readonly property bool taskbarStyle: BarMetrics.widgetStyle(barConfig) === "taskbar"
    ...
    readonly property real rt: {
        ...
        if (taskbarStyle || (barConfig?.squareCorners ?? false))
            return 0;
    ...
    readonly property bool shadowEnabled: !taskbarStyle && Theme.elevationEnabled && (SettingsData.barElevationEnabled ?? true)
    ...
    Rectangle {
        id: taskbarHorizontalEdge
        readonly property real thickness: Theme.hairline(CompositorService.getScreenScale(root.barWindow.screen))
        z: 90
        visible: root.taskbarStyle && !root.axis.isVertical
        x: body.x
        y: root.isBottom ? body.y : body.y + body.height - thickness
        width: body.width
        height: thickness
    ...
    Rectangle {
        id: taskbarVerticalEdge
        readonly property real thickness: Theme.hairline(CompositorService.getScreenScale(root.barWindow.screen))
        z: 90
        visible: root.taskbarStyle && root.axis.isVertical
        x: root.isRight ? body.x : body.x + body.width - thickness
        y: body.y
        width: thickness
        height: body.height
        color: Theme.withAlpha(Theme.surfaceText, 0.16)
