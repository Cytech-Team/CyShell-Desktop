...
Item {
    ...
    // Taskbar style deliberately renders no bar shadow; keeping the old invisible
    // buffer creates needless layer-surface headroom and makes pointer geometry
    // harder for compositors to reason about.
    readonly property bool _shadowActive: (renderBarConfig?.widgetStyle ?? "pills") !== "taskbar" && Theme.elevationEnabled && (typeof SettingsData !== "undefined" ? (SettingsData.barElevationEnabled ?? true) : false)
    ...
    Item {
        ...
        MouseArea {
            ...
            HoverHandler {
                onHoveredChanged: {
                    if (hovered)
                        CompositorService.noteScreenInteraction(barWindow.screen);
                }
            }
