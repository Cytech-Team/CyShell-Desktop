...
function migrateToVersion(obj, targetVersion) {
    ...
    if (currentVersion < 29 && targetVersion >= 29) {
        ...
        for (const bc of bars) {
            if (!bc || bc.island === true)
                continue;
            if (!Array.isArray(bc.rightWidgets))
                bc.rightWidgets = [];
            const hasAgent = bc.rightWidgets.some(widget => {
                const id = typeof widget === "string" ? widget : (widget?.id || widget?.widgetId || "");
                return id === "agentStatus";
            });
            if (!hasAgent) {
                const insertBefore = bc.rightWidgets.findIndex(widget => {
                    const id = typeof widget === "string" ? widget : (widget?.id || widget?.widgetId || "");
                    return id === "notificationButton" || id === "controlCenterButton";
                });
                if (insertBefore >= 0)
                    bc.rightWidgets.splice(insertBefore, 0, "agentStatus");
                else
                    bc.rightWidgets.push("agentStatus");
            }
        }
