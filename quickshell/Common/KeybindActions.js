...
const DMS_ACTIONS = [
    ...
    { id: "spawn dms agent open", label: "Agent: Open" },
    { id: "spawn dms agent review", label: "Agent: Review Permissions" },
    { id: "spawn dms agent stop", label: "Agent: Emergency Stop" },
...
function isDmsAction(action) {
    if (!action)
        return false;
    return action.startsWith("spawn dms ipc call ") || /^spawn dms screenshot( |$)/.test(action) || /^spawn dms agent( |$)/.test(action);
}
