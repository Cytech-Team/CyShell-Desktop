...
DankFloatingWindow {
    ...
    // labwc may map a Quickshell FloatingWindow at its default tiny size before
    // minimumSize/maximumSize constraints are applied. Give the surface an
    // explicit initial geometry so the greeter never flashes/opens as a tiny window.
    width: modalWidth
    height: modalHeight
