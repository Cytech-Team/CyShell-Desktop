...
Singleton {
    ...
    signal workspaceStateUpdate(var data)
    ...
    signal agentEvent(var data)

    property bool capsLockState: false
    property bool screensaverInhibited: false
    property var screensaverInhibitors: []

    property var activeSubscriptions: ["network", "network.credentials", "loginctl", "freedesktop", "freedesktop.screensaver", "gamma", "theme.auto", "wallpaper", "bluetooth", "bluetooth.pairing", "brightness", "wlroutput", "workspace", "evdev", "browser", "dbus", "clipboard", "sysupdate", "agent"]
    ...
    function removeSubscription(service) {
        if (activeSubscriptions.includes("all") && service !== "mpris.command") {
            const allServices = ["network", "loginctl", "freedesktop", "gamma", "bluetooth", "brightness", "browser", "location", "agent"];
    ...
    function handleSubscriptionEvent(response) {
        if (response.error) {
            if (response.error.includes("unknown method") && response.error.includes("subscribe")) {
                if (!shownOutdatedError) {
                    ...
                    ToastService.showError(I18n.tr("CyShell out of date"), I18n.tr("To update, run the following command:"), updateCommand);
        ...
        if (service === "server") {
            ...
            if (apiVersion < expectedApiVersion) {
                ToastService.showError(I18n.tr("CyShell backend is outdated (API v%1, expected v%2)", "error toast, %1 is current api version, %2 is required version").arg(apiVersion).arg(expectedApiVersion));
            }
    ...
    function sendRequest(method, params, callback, timeoutMs) {
        if (!isConnected) {
            ...
            if (callback) {
                callback({
                    "error": "not connected to CyShell backend socket"
                });
            }
            return;
        }
