...
Singleton {
    ...
    function searchForAgent(text, maxResults) {
        const parsed = parseInt(maxResults || "15");
        const limit = Math.max(1, Math.min(50, isNaN(parsed) ? 15 : parsed));
        return _searchEntries(text, limit).map(entry => {
            const section = String(entry.section || "");
            const hasSetting = section !== "" && SettingsData.hasSetting(section);
            let valueType = "";
            let value = null;
            let writable = false;
            if (hasSetting) {
                const current = SettingsData[section];
                valueType = typeof current;
                writable = valueType === "boolean" || valueType === "number" || valueType === "string";
                if (writable)
                    value = current;
            }
            return {
                section: section,
                settingKey: hasSetting ? section : "",
                label: entry.label || "",
                category: entry.category || "",
                description: entry.description || "",
                page: entry.page || SettingsTabs.pageForTabIndex(entry.tabIndex) || "",
