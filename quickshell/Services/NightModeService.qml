...
Singleton {
    ...
    function enableNightMode() {
        if (!gammaControlAvailable) {
            ToastService.showWarning(I18n.tr("Night mode failed: CyShell gamma control not available"));
            return;
        }
    ...
    IpcHandler {
        ...
        function gamma(value: string): string {
            if (!root.gammaAdjustAvailable)
                return "Gamma adjustment not available (requires CyShell API v34+)";
            if (!value)
        ...
        function contrast(value: string): string {
            if (!root.gammaAdjustAvailable)
                return "Contrast adjustment not available (requires CyShell API v34+)";
            if (!value)
