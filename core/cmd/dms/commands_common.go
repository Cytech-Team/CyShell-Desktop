...
var ipcCmd = &cobra.Command{
	Use:   "ipc",
	Short: "Send IPC commands to the running CyShell shell",
	Long: `Send IPC commands to the running CyShell shell.

  cyshell ipc call <target> <function> [args...]   invoke a command
  cyshell ipc list                                 list all targets and functions

The legacy dms command remains a compatibility alias.`,
...
var debugSrvCmd = &cobra.Command{
	...
	Long:  "Start the Unix socket debug server for CyShell",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsCmd = &cobra.Command{
	Use:   "plugins",
	Short: "Manage CyShell plugins",
	Long:  "Browse and manage CyShell plugins from the registry",
}

var pluginsBrowseCmd = &cobra.Command{
	Use:   "browse",
	Short: "Browse available plugins",
	Long:  "Browse available plugins from the plugin registry",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsListCmd = &cobra.Command{
	...
	Long:  "List all installed CyShell plugins",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsInstallCmd = &cobra.Command{
	...
	Long:  "Install a CyShell plugin from the registry using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
...
var pluginsUninstallCmd = &cobra.Command{
	...
	Long:  "Uninstall a CyShell plugin using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
var pluginsUpdateCmd = &cobra.Command{
	...
	Long:  "Update an installed CyShell plugin using its ID (e.g., 'myPlugin'). If --all or -a is specified, all installed plugins will be updated.",
...
// Git builds: CyShell (git) v0.6.2-XXXX
// Stable releases: CyShell v0.6.2
func formatVersion(version string) string {
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("CyShell (git) v%s-%s", matches[1], matches[2])
	}
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		...
		return fmt.Sprintf("CyShell (git) v%s-%s", baseVersion, matches[1])
	}

	// Stable release format: 0.6.2
	re = regexp.MustCompile(`^([\d.]+)$`)
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("CyShell v%s", matches[1])
	}

	return fmt.Sprintf("CyShell %s", version)
}
...
func getCommonCommands() []*cobra.Command {
	...
	for _, command := range commands {
		switch command.Name() {
		case "run":
			command.Short = "Launch CyShell (backend + UI)"
			command.Long = "Start the CyShell backend and launch the desktop UI."
		case "restart":
			command.Short = "Restart the running CyShell instance"
		case "kill":
			command.Short = "Kill running CyShell processes"
		}
	}
