...
var updateCmd = &cobra.Command{
	...
	Long:  "CyShell development builds do not use the legacy upstream DMS release updater. Update from the CyShell repository/release channel instead.",
...
var updateCheckCmd = &cobra.Command{
	...
	Long:  "Report the update policy for this CyShell development build without contacting the legacy upstream release channel.",
	Run: func(cmd *cobra.Command, args []string) {
		printCyShellUpdateGuidance()
	},
}
...
func printCyShellUpdateGuidance() {
	...
	fmt.Println()
	fmt.Println("The legacy DMS package/release updater is disabled in CyShell so it cannot overwrite this fork with upstream binaries.")
}

func runUpdateCheck() {
	fmt.Println("Checking for CyShell updates...")
	fmt.Println()
...
func runUpdate() {
	...
	if updateErr != nil {
		...
		log.Fatalf("Error updating CyShell: %v", updateErr)
	}

	log.Info("Update complete! Restarting CyShell...")
	shellApp.Restart()
}
...
func updateArchLinux() error {
	...
	fmt.Printf("This will update CyShell using %s.\n", helper)
	if !confirmUpdate() {
		return errdefs.ErrUpdateCancelled
	}
...
func updateOtherDistros() error {
	...
	if _, err := os.Stat(dmsPath); os.IsNotExist(err) {
		return fmt.Errorf("CyShell compatibility configuration directory not found at %s", dmsPath)
	}

	fmt.Printf("Found CyShell compatibility configuration at %s\n", dmsPath)
	...
	fmt.Println("  2. CyShell compatibility configuration using git")
	if !confirmUpdate() {
		return errdefs.ErrUpdateCancelled
	}
	...
	fmt.Println("\n=== Updating CyShell shell configuration ===")

	if err := os.Chdir(dmsPath); err != nil {
		return fmt.Errorf("failed to change to CyShell compatibility directory: %w", err)
	}
	...
	if currentTag != "" {
		...
		if hasLocalChanges {
			fmt.Println("\nWarning: You have local changes in your CyShell compatibility configuration.")
