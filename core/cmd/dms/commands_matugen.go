...
var matugenCmd = &cobra.Command{
	...
	Long:  "Generate Material Design themes using matugen with CyShell Base16 palette integration",
}
...
var matugenQtengineCmd = &cobra.Command{
	Use:   "qtengine",
	Short: "Sync qtengine's config with the current CyShell theme",
...
func init() {
	...
	for _, cmd := range []*cobra.Command{matugenGenerateCmd, matugenQueueCmd} {
		...
		cmd.Flags().String("shell-dir", "", "CyShell shell installation directory")
