...
var openCmd = &cobra.Command{
	...
	Long: `Open a target (URL, file, or other resource) using the CyShell application picker.
...
Examples:
  cyshell open https://example.com                    # Open URL with browser picker
  cyshell open file.pdf                               # Open file (MIME auto-detected)
  cyshell open file.pdf --mime application/pdf        # Override MIME detection
  cyshell open document.odt --category Office         # Open with office applications`,
...
func runOpen(target string) {
	...
	if err := sendServerRequestFireAndForget(req); err != nil {
		fmt.Println("CyShell is not running. Please start CyShell first.")
		os.Exit(1)
	}
