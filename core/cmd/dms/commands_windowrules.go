...
var windowrulesListCmd = &cobra.Command{
	...
	Long:  "List all window rules from compositor config file. Returns JSON with rules and CyShell status.",
...
var windowrulesAddCmd = &cobra.Command{
	...
	Short: "Add a window rule to the CyShell-managed file",
	Long:  "Add a new window rule to the CyShell-managed rules file.",
...
var windowrulesUpdateCmd = &cobra.Command{
	...
	Short: "Update a window rule in the CyShell-managed file",
	Long:  "Update an existing window rule in the CyShell-managed rules file.",
...
var windowrulesRemoveCmd = &cobra.Command{
	...
	Short: "Remove a window rule from the CyShell-managed file",
	Long:  "Remove a window rule from the CyShell-managed rules file.",
	Args:  cobra.ExactArgs(2),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) == 0 {
			return []string{"hyprland", "niri", "mango"}, cobra.ShellCompDirectiveNoFileComp
...
var windowrulesReorderCmd = &cobra.Command{
	...
	Short: "Reorder window rules in the CyShell-managed file",
