...
func logStartupFailure(exitCode int, uptime time.Duration, stderrTail string) {
	...
	if containsFontCrashSignature(stderrTail) {
		log.Errorf("CyShell startup failed due to a potential font/rendering crash. Try running 'fc-cache -fv' and restarting CyShell.")
	} else {
		log.Errorf("CyShell startup failed (exit code %d). Run 'cyshell doctor' for more diagnostics.", exitCode)
	}
}
