...
Input:
  cyshell qr "https://example.com"     # encode an argument
  echo -n "data" | dms qr          # encode stdin
  cyshell qr -                         # encode stdin explicitly

Output (combine freely):
  cyshell qr "text" --clipboard        # copy PNG image to clipboard
  cyshell qr "text" --copy-text        # copy the source text to clipboard
  cyshell qr "text" -o code.png        # save a PNG file
  cyshell qr "text" > code.png         # PNG to stdout (piped)
  cyshell qr "text" --render           # force terminal render
...
WiFi:
  cyshell qr wifi MySSID -p secret     # build from an explicit password
  cyshell qr wifi MySSID               # pull the saved secret from the shell`,
...
credentials are fetched from the running CyShell shell (like the network panel).`,
