...
// XML file : internal/proto/xml/ext-workspace-v1.xml
//
...
func (i *ExtWorkspaceManagerV1) Dispatch(opcode uint32, fd int, data []byte) {
	...
	case 0:
		...
		l := 0
		workspaceGroupID := client.Uint32(data[l : l+4])
		if workspaceGroup, ok := i.Context().GetProxy(workspaceGroupID).(*ExtWorkspaceGroupHandleV1); ok && !workspaceGroup.IsZombie() {
			e.WorkspaceGroup = workspaceGroup
		} else if workspaceGroupID != 0 {
			workspaceGroup := &ExtWorkspaceGroupHandleV1{}
			i.Context().RegisterWithID(workspaceGroup, workspaceGroupID)
			e.WorkspaceGroup = workspaceGroup
		}
		l += 4
	...
	case 1:
		...
		if workspace, ok := i.Context().GetProxy(workspaceID).(*ExtWorkspaceHandleV1); ok && !workspace.IsZombie() {
			e.Workspace = workspace
		} else if workspaceID != 0 {
			workspace := &ExtWorkspaceHandleV1{}
			i.Context().RegisterWithID(workspace, workspaceID)
			e.Workspace = workspace
		}
		l += 4
...
func (i *ExtWorkspaceGroupHandleV1) Dispatch(opcode uint32, fd int, data []byte) {
	...
	case 1:
		...
		if output, ok := i.Context().GetProxy(outputID).(*client.Output); ok && !output.IsZombie() {
			e.Output = output
		} else if outputID != 0 {
			output := &client.Output{}
