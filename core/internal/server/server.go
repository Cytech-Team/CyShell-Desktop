...
var cycomManager *cycom.Manager
...
func InitializeCyComManager() error {
	manager, err := cycom.NewManager()
	if err != nil {
		log.Warnf("Failed to initialize CyCom runtime: %v", err)
		return err
	}
	cycomManager = manager
	log.Info("CyCom agent runtime initialized in-process")
	return nil
}
...
func InitializeExtWorkspaceManager() error {
	log.Info("Attempting to initialize ext-workspace management...")
	...
	manager, err := extworkspace.NewManager(wlContext)
	if err != nil {
		return err
	}
	extWorkspaceManager = manager
	log.Info("Ext-workspace management initialized")
	return nil
}
...
func handleSubscribe(ctx context.Context, conn *ipc.ConnWriter, req ipc.Request) {
	...
	if shouldSubscribe("agent") && cycomManager != nil {
		mgr := cycomManager
		id := clientID + "-agent"
		source := mgr.Subscribe(id)
		forwardSubscription(&wg, eventChan, stopChan, "agent", source, func() {
			mgr.Unsubscribe(id)
		}, mgr.SnapshotEvent)
	}
	...
	if shouldSubscribe("workspace") && extWorkspaceManager != nil {
		mgr := extWorkspaceManager
		id := clientID + "-workspace"
		source := mgr.Subscribe(id)
		forwardSubscription(&wg, eventChan, stopChan, "workspace", source, func() {
			mgr.Unsubscribe(id)
		}, mgr.GetState)
	}
...
func (s *Server) Serve(printDocs bool) error {
	...
	if err := InitializeExtWorkspaceManager(); err != nil {
		log.Debugf("Ext-workspace manager unavailable: %v", err)
	} else {
		notifyCapabilityChange()
	}
	...
	if err := InitializeCyComManager(); err != nil {
		log.Warnf("CyCom runtime unavailable: %v", err)
	} else {
		notifyCapabilityChange()
	}
