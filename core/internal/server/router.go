...
func newRequestMux() *ipc.Mux {
	...
	mux.HandlePrefix("cycom.", func(ctx context.Context, conn *ipc.ConnWriter, req ipc.Request, _ *ipc.Subscriber) {
		if cycomManager == nil {
			models.RespondError(conn, req.ID, "CyCom runtime not initialized")
			return
		}
		cycom.HandleRequest(ctx, conn, req, cycomManager)
	})
	...
	mux.HandlePrefix("workspace.", requestHandler(func(conn *ipc.ConnWriter, req ipc.Request) {
